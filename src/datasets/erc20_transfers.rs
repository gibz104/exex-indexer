...
pub async fn process_erc20_transfers<Node: FullNodeComponents, EthApi: FullEthApi>(
    ...
    for (tx_idx, (tx, receipt)) in block.body().transactions.iter().zip(receipts).enumerate() {
        for (log_idx, log) in receipt.logs.iter().enumerate() {
            ...
            match Transfer::decode_raw_log(log.topics(), &log.data.data) {
                Ok(transfer) => {
                    writer
